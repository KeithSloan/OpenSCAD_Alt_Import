...
#*                                                                         *
#***************************************************************************
__title__="FreeCAD OpenSCAD Workbench - Parse SCAD to FreeCAD Sheet"
...
def parseFunction(filename):
    from scadParser.scad_parser import parseFileAndPrintGlobals
    print(f"Parse File {filename}")
    parseFileAndPrintGlobals(filename)

def parse2SheetFunction(filename, doc):
    from scadParser.scad_parser import parseFileAndPrintGlobals
    print(f"Parse to Sheet {filename}")
    parseFileAndPrintGlobals(filename)
...
def open(filename, currentdoc=None):
    ...
    print(f"SCAD Parse to Spreadsheet")
    parseFunction(filename)
