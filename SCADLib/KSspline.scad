...
function bspline(controlPoints, knots, path, closed=false, splinesteps=8, degree=3) =
    assert(is_int(degree))
    assert(is_int(controlPoints))
    ...
    m = len(knots) - 1
    p = degree
    assert(m = n + p + 1),"Incorrect number of Knots"
    let(
...
cp =
knots = [0, 0, 0, 0, 0.14, 0.28, 0.42, 0.57, 0.71, 0.85, 1, 1, 1, 1]
#bspline(controlPoints, knots, path, closed=false, splinesteps=8, degree=3);
bspline(controlPoints, knots, path, closed=false, splinesteps=8, degree=3);
